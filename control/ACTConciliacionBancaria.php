...
include_once(dirname(__FILE__) . '/../reportes/RConciliacionBancariaNuevoPDF.php');
include_once(dirname(__FILE__) . '/../reportes/RConciliacionBancariaDetalleNuevoPDF.php');
...
class ACTConciliacionBancaria extends ACTbase
{
    ...
    function reporteConciliacionBancaria()
    {
        ...
        $this->objFunc = $this->create('MODConciliacionBancaria');
        $this->res2 = $this->objFunc->reporteExtracto($this->objParam);
        ...
        $this->objReporteFormato->setDatos($this->res->datos, $this->res1->datos, $this->res2->datos);
    ...
    public function detalleConciliacionBancariaNuevo()
    {
        // Validar que haya conciliación seleccionada
        if ($this->objParam->getParametro('id_conciliacion_bancaria') != '') {
            $this->objParam->addFiltro(
                "detcon.id_conciliacion_bancaria = " . $this->objParam->getParametro('id_conciliacion_bancaria')
            );
        }

        if ($this->objParam->getParametro('tipoReporte') == 'pdf') {
            $this->objFunc = $this->create('MODConciliacionBancaria');
            $datos = $this->objFunc->listarDetalleConciliacionBancariaNuevo($this->objParam)->getDatos();

            $nombreArchivo = 'Conciliacion_' . uniqid(md5(session_id())) . '.pdf';

            $this->objParam->addParametro('nombre_archivo', $nombreArchivo);

            $this->objReporte = new RConciliacionBancariaNuevoPDF($this->objParam);
            $this->objReporte->setDatos($datos);
            $this->objReporte->generarReporte();

            $this->objReporte->Output($this->objReporte->url_archivo, 'F');

            $mensajeExito = new Mensaje();
            $mensajeExito->setMensaje(
                'EXITO', 'Reporte.php', 'Reporte generado', 'Se generó con éxito el reporte', 'control'
            );
            ...
            $this->objFunc = $this->create('MODConciliacionBancaria');
            $this->res = $this->objFunc->listarDetalleConciliacionBancariaNuevo($this->objParam);
            $this->res->imprimirRespuesta($this->res->generarJson());
        }
    }
    ...
    function detalleConciliaRepoNuevo()
    {
        ...
        if ($this->objParam->getParametro('tipoReporte') == 'pdf') {
            ...
            $this->objFunc = $this->create('MODConciliacionBancaria');
            $datos = $this->objFunc->detalleConciliaRepoNuevo($this->objParam)->getDatos();

            $nombreArchivo = 'Conciliacion_' . uniqid(md5(session_id())) . '.pdf';
            $this->objParam->addParametro('nombre_archivo', $nombreArchivo);

            $this->objReporte = new RConciliacionBancariaDetalleNuevoPDF($this->objParam);
            $this->objReporte->setDatos($datos);
            $this->objReporte->generarReporte();
            ...
            // Si no es PDF, retornamos JSON
            $this->objFunc = $this->create('MODConciliacionBancaria');
            $this->res = $this->objFunc->detalleConciliaRepoNuevo($this->objParam);
            $this->res->imprimirRespuesta($this->res->generarJson());
        }
    }

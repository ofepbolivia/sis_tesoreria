...
BEGIN
		...
		begin
           ...
           IF  v_id_funcionario_sol is not NULL  THEN
			...
			update tes.tobligacion_pago set
              ...
              id_funcionario_responsable = v_parametros.id_funcionario_responsable
	...
	***********************************/
	...
	elseif(p_transaccion='TES_SIGESTOB_IME')then
        ...
        */
       /*(F.E.A) Se crea la funcion tes.f_sig_stado_ob con el objetivo de que se pueda pasar
         la obligacion de pago a el estado en pago para crear las cuotas del plan_de_pago
         todo el codigo contenido de la transaccion TES_SIGESTOB_IME.
       */
       IF( tes.f_sig_stado_ob(p_administrador,p_id_usuario, hstore(v_parametros)) )THEN
            	RAISE NOTICE 'EXITO';
       END IF;
        --comentamos este codigo porque todo su contenido lo tenemos en la  funcion tes.f_sig_stado_ob

	...
	if(p_transaccion='TES_SOLEFE_SEL')then
    	...
    	begin
            ...
            IF  lower(v_parametros.tipo_interfaz) in ('condetalle','sindetalle','efectivocaja') THEN
                ...
                select id_tipo_solicitud into v_id_tipo_reposicion
                from tes.ttipo_solicitud
                where codigo='REPEFE';
            ...
            IF lower(v_parametros.tipo_interfaz) in ('vbsolicitudefectivo') THEN
            	...
            	IF p_administrador !=1 THEN
                   v_filtro = '(ew.id_funcionario='||v_parametros.id_funcionario_usu::varchar||' ) and  (solefe.estado in (''vbjefe'',''vbcajero'',''vbfin'')) and  solefe.id_tipo_solicitud in (select id_tipo_solicitud from tes.ttipo_solicitud where codigo in (''SOLEFE'',''REPEFE'')) and ';
                 ELSE
                     v_filtro = '(solefe.estado in (''vbjefe'',''vbcajero'',''vbfin'')) and
                     solefe.id_tipo_solicitud in (select id_tipo_solicitud from tes.ttipo_solicitud where codigo in (''SOLEFE'',''REPEFE'')) and ';
                END IF;
            END IF;
			...
			v_consulta:='select
                        ...
                        else caja.dias_maximo_rendicion - (CURRENT_DATE - COALESCE(solefe.fecha_entrega,current_date) - pxp.f_get_weekend_days(COALESCE(solefe.fecha_entrega,current_date),current_date))::integer end as dias_no_rendidos,
	...
	***********************************/
    	...
    	begin
			...
			v_consulta:='select sol.fecha_entrega as fecha,
                                  ''Recibo de BOA'' as desc_plantilla,
                                  NULL as rendicion,

...
BEGIN
	...
	***********************************/
		...
		begin
            ...
            /*Preparamos la alarma para la ultima cuota para enviar al funcionario Auxiliar de Adquisiciones,
            indicando que es la ultima cuota, y que debe adjuntar form 500.*/
            select count(tp.id_plan_pago)
            into v_cont
            from tes.tplan_pago tp
            where tp.id_plan_pago = v_parametros.id_plan_pago and tp.es_ultima_cuota;
            v_cont = COALESCE(v_cont, 0);
            if(v_cont>=1)then
              select
               ...
               vf.email_empresa,
               vf.id_funcionario,
               vfs.desc_funcionario1,
               top.id_obligacion_pago
              into
              v_registros_aux
              from tes.tplan_pago tpp
              inner join tes.tobligacion_pago top on top.id_obligacion_pago = tpp.id_obligacion_pago
              inner join adq.tproceso_compra tpc on tpc.num_tramite = top.num_tramite
              inner join segu.tusuario tu on tu.id_usuario = tpc.id_usuario_auxiliar
              INNER JOIN orga.tfuncionario tf on tf.id_persona = tu.id_persona
              inner join orga.vfuncionario_persona vf on vf.id_funcionario = tf.id_funcionario
              inner join adq.tsolicitud ts on ts.num_tramite = top.num_tramite
              inner join orga.vfuncionario_persona vfs on vfs.id_funcionario = ts.id_funcionario
              where tpp.id_plan_pago = v_parametros.id_plan_pago and tpp.es_ultima_cuota;


              v_descripcion =  'Estimad@, '|| v_registros_aux.desc_funcionario1||'<br>'||
              'para el tramite #'||v_registros_aux.num_tramite||'<br>se ha dado conformidad a la ultima cuota.<br>'||
              'para dar seguimiento, por favor adjuntar formulario 500 .';
              ...
              v_id_alarma :=  param.f_inserta_alarma(
                                                    v_registros_aux.id_funcionario,
                                                    v_descripcion,    --descripcion alarmce
                                                    '../../../sis_tesoreria/vista/obligacion_pago/ObligacionPagoAdq.php',--acceso directo
                                                    now()::date,
                                                    'notificacion',
                                                    '',   -->
                                                    p_id_usuario,
                                                    ...
                                                    '{filtro_directo:{campo:"id_obligacion_pago",valor:"'||v_registros_aux.id_obligacion_pago::varchar||'"}}',
                                                    NULL::integer,
                                                    'Se habilito pago al tramite #'||v_registros_aux.num_tramite
                                                    );

              RAISE NOTICE 'v_id_alarma: %',v_id_alarma;
			end if;
  ...
  /*********************************
	...
	***********************************/
        ...
        /*********************************
      #TRANSACCION:  'TES_ULTIMA_CUOTA_GET'
      #DESCRIPCION:	Verificamos si la obligacion de pago tiene ultima cuota.
      #AUTOR:		Franklin E.
      #FECHA:		31-05-2016
      ***********************************/

      elsif(p_transaccion='TES_ULTIMA_CUOTA_GET')then

        begin
        	select count(tpp.id_plan_pago)
            into v_cont
            from tes.tplan_pago tpp
            where tpp.id_obligacion_pago = v_parametros.id_obligacion_pago and tpp.es_ultima_cuota;
            v_cont = coalesce(v_cont,0);
        	IF(v_cont>=1)THEN
        		v_bandera = true;
            ELSE
            ...
            END IF;
            --Definicion de la respuesta
            v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Se obtiene la ultima cuota con Exito');
